...
public class ProductService : IProductService
{
    ...
    public async Task<Product> CreateProductAsync(CreateProductDto productDto)
    {
        ...
        var newProduct = new Product(productDto.Name, productDto.Description, productDto.Category, productDto.UnitPrice,
            productDto.QuantityInStock, productDto.ReorderLevel);
    ...
    public async Task<PagedResult<ProductDto>> SearchProductsAsync(SearchDto searchDto)
    {
        ...
        if (!string.IsNullOrEmpty(searchDto.SearchTerm))
            productsQuery = productsQuery.Where(p =>
                p.Name.Contains(searchDto.SearchTerm) || p.Description.Contains(searchDto.SearchTerm));
        ...
        if (!string.IsNullOrEmpty(searchDto.SortBy))
            productsQuery = searchDto.SortBy switch
            {
                ...
                "unitPrice" => !searchDto.SortDescending
                    ? productsQuery.OrderBy(p => p.UnitPrice)
                    : productsQuery.OrderByDescending(p => p.UnitPrice),
                "quantityInStock" => !searchDto.SortDescending
                    ? productsQuery.OrderBy(p => p.QuantityInStock)
                    : productsQuery.OrderByDescending(p => p.QuantityInStock),
                "reorderLevel" => !searchDto.SortDescending
                    ? productsQuery.OrderBy(p => p.ReorderLevel)
                    : productsQuery.OrderByDescending(p => p.ReorderLevel),
    ...
    public async Task<ProductDto> UpdateProductAsync(int id, UpdateProductDto productDto)
    {
        ...
        product.Update(productDto.Name, productDto.Description, productDto.Category, productDto.UnitPrice,
            productDto.ReorderLevel);
