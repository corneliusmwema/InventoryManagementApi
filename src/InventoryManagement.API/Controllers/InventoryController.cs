...
public class InventoryController : ControllerBase
{
    ...
    public async Task<IActionResult> GetTransactionHistoryForProductAsync(int productId)
    {
        try
        {
            ...
            _logger.LogInformation("Started getting transaction history for product with id {ProductId} at {StartTime}",
                productId, startTime);
            ...
            _logger.LogInformation(
                "Completed getting transaction history for product with id {ProductId} in {Duration}", productId,
                duration.TotalMilliseconds);

            return Ok(ApiResponse<IEnumerable<InventoryTransactionDto>>.SuccessWithData(result,
                "Transaction history retrieved successfully", 200));
        }
        catch (ApiException)
        {
            // The middleware will handle this
